            ...
            return h('div', [
              h('i-switch', {
                ...
                on: {
                  'on-change': value => {
                    // 启用/禁用
                    this.api.put(`/api/admin/product_category/${params.row.id}/`, {status: ~~value}).then(res => {
                      if (res.data.code) {
            ...
            return h('div', [
              ...
              h('a', [
                h('Poptip', {
                  ...
                  on: {
                    'on-ok': () => {
                      this.api.delete(`/api/admin/product_category/${params.row.id}/`).then(res => {
                        if (res.data.code) {
                          this.$Message.error(res.data.msg);
                        } else {
        ...
        this.$refs.editCategoryForm.validate(valid => {
          if (valid) {
            if(this.handleType) { //编辑
              this.api.put('/api/admin/product_category/' + this.editCategoryFormData.id + '/', {
              ...
              this.api.post('/api/admin/product_category/', {
                category_name: this.editCategoryFormData.category_name,
                parent_id: this.editCategoryFormData.parent_id,
                status: this.editCategoryFormData.status === '启用' ? 1 : 0
              }).then(res => {
